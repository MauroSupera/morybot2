...
const removerFundo = (bufferImagem) => {
  return new Promise(async(resolve,reject) => {
   try {
    var resposta = {};
    var nomeArquivo = crypto.randomBytes(10).toString('hex') + '.png';
    var data = new FormData();
    data.append('files', bufferImagem, {filename: nomeArquivo})
    var respostaUpload = await axios.request({
        method: 'post',
    ...
    var dadosUpload = JSON.parse(JSON.stringify(respostaUpload.data))
    ...
    var respostaFotoUrl = await axios.request({
        method: 'post',
        maxBodyLength: Infinity,
    ...
    var fotoUrl = respostaFotoUrl.data.match(/https:\/\/download1\.imageonline\.co\/download\.php\?filename=[A-Za-z0-9]+-imageonline\.co-[0-9]+\.png/m)
